...
// Remove only chunks generated by this script's earlier local build. The
// original Hugging Face cache is preserved; the site now streams fixed ranges.
for (let index = 0; index < 21; index++) {
  await unlink(path.join(modelDir, `part-${String(index).padStart(2, '0')}.bin`)).catch(error => { if (error.code !== 'ENOENT') throw error; });
}
